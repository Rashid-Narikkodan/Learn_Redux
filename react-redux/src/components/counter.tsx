...
const Counter = () => {
  ...
  return (
    <div className="card flex gap-2">
      <button className="rounded-md p-4 border border-red-300 " onClick={() => dispatch({ type: "dec" })}>-</button>
      <p className="border p-5 rounded">count is {typeof value == "number" && value}</p>
      <button className="rounded-md p-4 border border-green-300"  onClick={() => dispatch({ type: "inc" })}>+</button>
      <p></p>
    </div>
  );
};
